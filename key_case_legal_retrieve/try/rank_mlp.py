...
class Selector1(nn.Module):
    def __init__(self, input_size, filters, kernel_size, dilation_rate):
        ...
        # ]
        # self.net1 = nn.Sequential(*list_layers_1)
        # self.net2 = nn.Sequential(*list_layers_2)
        self.dense1 = nn.Linear(input_size, filters, bias=False)
        self.dense11 = nn.Linear(input_size, filters, bias=False)
        # self.dense111 = nn.Linear(input_size, filters, bias=False)
        self.dense2 = nn.Linear(filters, 1,)
        self.dense22 = nn.Linear(filters, 1,)
        # self.dense222 = nn.Linear(filters, 1,)
        self.dense3 = nn.Linear(2, 1)
        self.dense4 = nn.Linear(2, 1)
        self.dense5 = nn.Linear(2, 1)

    def forward(self, inputs, labels=None, type='train'):
        inputs_sep = [inputs[..., :768], inputs[..., 768:1536], inputs[..., 1536], inputs[..., 1537]]
        # key_out = self.net1(inputs_sep[0])
        # case_out = self.net2(inputs_sep[1])
        # key_out = nn.Dropout(0.1)(inputs_sep[0])
        # a = (key_out != inputs_sep[0]).long()
        if type == 'train':
            key_out = self.dense1(nn.Dropout(0.1)(inputs_sep[0]))
            key_out = nn.ReLU()(key_out)
            # key_out = self.net1(inputs_sep[0])
            key_out = self.dense2(nn.Dropout(0.1)(key_out))
            case_out = self.dense11(nn.Dropout(0.1)(inputs_sep[1]))
            case_out = nn.ReLU()(case_out)
            # case_out = self.net2(inputs_sep[1])
            case_out = self.dense22(nn.Dropout(0.1)(case_out))
            # dis_out = self.dense11(nn.Dropout(0.1)(inputs_sep[2]))
            # dis_out = nn.ReLU()(dis_out)
            # dis_out = self.dense22(nn.Dropout(0.1)(dis_out))
        else:
            key_out = self.dense1((inputs_sep[0]))
            key_out = nn.ReLU()(key_out)
            # key_out = self.net1(key_out)
            key_out = self.dense2((key_out))
            case_out = self.dense11((inputs_sep[1]))
            case_out = nn.ReLU()(case_out)
            # case_out = self.net2(case_out)
            case_out = self.dense22((case_out))
            dis_out = None
        key_logits, case_logits = inputs_sep[2].unsqueeze(2), inputs_sep[3].unsqueeze(2)
        # output = self.dense3(torch.cat([key_out, key_logits, case_logits, case_out], -1))
        key_out = self.dense4(torch.cat([key_logits, key_out], -1))
        case_out = self.dense5(torch.cat([case_logits, case_out], -1))
        output = self.dense3(torch.cat([key_out, case_out], -1))
...
class Selector_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        # old_data = self.data_x_tensor[idx].to(device)
        # new_data = self.dis_model(old_data.unsqueeze(0))
        # data_x = torch.cat([old_data, new_data.squeeze(0)], dim=-1)
...
def ndcg_listwise(scores, labels):
    labels = labels.squeeze()
    scores = scores.squeeze()
    qidx, dids, true_score = labels[:, 0], labels[:, 1], labels[:, 2]
    rank_ids = scores.argsort().flip(0)
    pre_rank_score = torch.index_select(true_score, 0, rank_ids)
    assert torch.div(qidx.sum(0), qidx.shape[0]) == qidx[0]

    dcg_value = torch.tensor(0.).to(device)
    idcg_value = torch.tensor(0.).to(device)
    sranks = pre_rank_score.argsort().flip(0)
    for i in range(0, 30):
        logi = torch.tensor(math.log(i + 2, 2)).to(device)
        dcg_value += torch.div(pre_rank_score[i], logi)
        idcg_value += torch.div(sranks[i], logi)
    if idcg_value.item() == 0.0:
        idcg_value += torch.tensor(0.00000001).to(device)
    return (1 - torch.div(dcg_value, idcg_value)) * 1000
...
def test_result(dataloader, args):
    ...
    with torch.no_grad():
        ...
        for data in tqdm(dataloader):
            ...
            assert len(set(qidx)) == 1
            all_preds[qidx[0]] = pred_ids[:30]
    json.dump(all_preds, open('result/prediction.json', "w", encoding="utf8"), indent=2,
              ensure_ascii=False, sort_keys=True)
    # fq = json.load(open('data/phase_1/train/label_top30_dict.json', 'r', encoding='utf8'))
    # ndcg_30 = cal_ndcg(all_preds, fq)
    # print(ndcg_30)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str, default="./result/4", help='checkpoint path')
    ...
    # filename = "./result/3/seed42-bsz1-lr5e-05/extract-loss7.pkl"
    # dis_model = torch.load(filename)
    # dis_model.eval().to(device)
    # test_result
    test_extract_json = './data/phase_2/test_data_win200.json'
    test_extract_npy = './data/phase_2/test_embeddings_s.npy'
    test_x = np.load(test_extract_npy)
    test_y = load_labels(test_extract_json, test_x, 'test')
    test_dataloader = DataLoader(Selector_Dataset(test_x, test_y), batch_size=1, shuffle=False)
    test_result(test_dataloader, args)
    exit(0)

    # training
    dev_extract_json = './data/phase_2/dev_data_win200.json'
    dev_extract_npy = './data/phase_2/dev_embeddings_s.npy'
    train_extract_json = './data/phase_2/train_data_win200.json'
    train_extract_npy = './data/phase_2/train_embeddings_s.npy'
    ...
    # train_y = np.concatenate([train_y, dev_y], axis=0)
    ...
    model = Selector1(args.input_size, args.hidden_size, kernel_size=args.kernel_size, dilation_rate=args.dilation_rate)
