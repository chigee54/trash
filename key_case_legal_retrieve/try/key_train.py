...
def main(seed):
    ...
    parser.add_argument('-train_file', default='data/key_train.json', type=str)
    parser.add_argument('-dev_file', default='data/key_dev.json', type=str)
    ...
    parser.add_argument('-max_length', default=2403, type=int)
...
class LongDataset(Dataset):
    def __init__(self, data_file, filter_qids=None, filter_cids=None):
        self.tokenizer = AutoTokenizer.from_pretrained("../pretrained_model/legal_roberta")
        self.filter_cids = filter_cids
        self.filter_qids = filter_qids.keys().append('3') if filter_qids else None
        if self.filter_cids and self.filter_qids:
            with open(data_file, 'r', encoding='utf8') as f:
                self.data = json.load(f)
        elif self.filter_cids and not self.filter_qids:
            with open(data_file, 'r', encoding='utf8') as f:
                data = json.load(f)
                for id, each in enumerate(data):
                    # if each['label'][0] in self.filter_ids:
                    if int(each['label'][1]) in self.filter_cids[each['label'][0]]:
                        data.remove(each)
                self.data = data
        elif self.filter_qids and not self.filter_cids:
            with open(data_file, 'r', encoding='utf8') as f:
                data = json.load(f)
                for id, each in enumerate(data):
                    if each['label'][0] in self.filter_qids:
                        data.remove(each)
                self.data = data
        else:
            with open(data_file, 'r', encoding='utf8') as f:
    ...
    def __getitem__(self, index):
        ...
        # features = self.tokenizer.encode_plus(data['query'], data['candidate'], max_length=self.max_length,
...
def train(model, args):
    ...
    train_data = LongDataset(args.train_file, all_labels)
