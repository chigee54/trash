...
class LongformerEncoder(nn.Module):
    ...
    def forward(
        ...
        is_global_attn = is_index_global_attn.flatten()
        # is_global_attn = is_global_attn.any().item()
...
class LongformerModel(LongformerPreTrainedModel):
    ...
    def _pad_to_window_size(
        ...
        if padding_len > 0:
            ...
            if gate_mask is not None:
                gate_mask = nn.functional.pad(gate_mask, (0, padding_len), value=pad_token_id)
    ...
    def forward(
        ...
        if gate_mask is None:
            gate_mask = torch.zeros(input_shape, device=device)

        # merge `global_attention_mask` and `attention_mask`
        if global_attention_mask is not None:
            attention_mask = self._merge_to_attention_mask(attention_mask, global_attention_mask)

        padding_len, input_ids, attention_mask, token_type_ids, position_ids, inputs_embeds, _ = self._pad_to_window_size(
            input_ids=input_ids,
            attention_mask=attention_mask,
