...
from transformers import AutoModel, AutoTokenizer, AdamW, get_linear_schedule_with_warmup, LongformerModel
...
def main():
    ...
    parser.add_argument('-train_file', default='data/phase_2/train_data_plus.json', type=str)
    parser.add_argument('-dev_file', default='data/phase_2/dev_data_plus.json', type=str)
...
class LongDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        query_cut = self.tokenizer.tokenize(data['query'])[:509]
        candidate_cut = self.tokenizer.tokenize(data['cpfxgc'])[:768]
        ...
        try:
            if data['labels'][2] == 1/3 or data['labels'][2] == 1.0:
                data['labels'][2] = 1
            else:
                data['labels'][2] = 0
        except:
