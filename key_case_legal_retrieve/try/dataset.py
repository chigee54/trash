...
class SegmentSelectionDataset(object):
    ...
    def encode(self, q_crime, d_crime, q, d, max_index, didx, labels, q_chunksize=100, d_chunksize=100, cutoff=True):  # q_chunksize=3, d_chunksize=15 for train
        ids, type_ids = [], []
        r_labels = []
        ...
        if labels is not None:
            r_labels.append(labels[didx] / 3 if didx in labels else 0)
        ...
        if max_index is not None:
            max_score_indexes = [0] + [i * len(q_chunks) + index[0] + 1 for i, index in enumerate(max_index) if len(index) > 0]
            j = 0
        ...
        return ids, type_ids, masks, len(q_chunks), len(d_chunks), r_labels
    ...
    def __getitem__(self, index):
        ...
        for doc in docs:
            ...
            token_ids, type_ids, token_masks, num_q, num_d, r_labels = self.encode(q_crime, d_crime, q, content, max_score_indexes, didx, labels)
            # try:
            ...
            select_token_ids.extend(token_ids)
            select_type_ids.extend(type_ids)
            select_token_masks.extend(token_masks)
            rel_labels.extend(r_labels)
    ...
    def get_predict_samples(self, q, q_crime, d):
        ...
        token_ids, type_ids, token_masks, num_q, num_d, _ = self.encode(q_crime, d_crime, q, content, None, None, None, cutoff=False)
