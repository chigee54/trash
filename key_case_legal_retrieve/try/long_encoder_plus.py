...
def main():
    ...
    parser.add_argument('-query_file', default='./data/phase_2/train/query.json', type=str)
    parser.add_argument('-candidate_file', default='./data/phase_2/train/candidates/', type=str)
    parser.add_argument('-saved_embeddings', default='./data/phase_2/train_embeddings_p', type=str)
    parser.add_argument('-saved_dev_embeddings', default='./data/phase_2/dev_embeddings_p', type=str)
    parser.add_argument('-saved_test_embeddings', default='./data/phase_2/test_embeddings_p', type=str)
    parser.add_argument('-saved_data', default='./data/phase_2/train_data_win200_plus.json', type=str)
    parser.add_argument('-saved_dev_data', default='./data/phase_2/dev_data_win200_plus.json', type=str)
    parser.add_argument('-saved_test_data', default='./data/phase_2/test_data_win200_plus.json', type=str)
...
class LongDataset(Dataset):
        ...
        for query in tqdm(queries, desc=u'数据转换'):
            ...
            for doc_file in doc_files:
                ...
                if label_top30_file:
                    ...
                    if qidx in self.filter_qids:
                        dev_data.append({'crime': crime, 'query': q, 'ajName': ajName, 'cpfxgc': cpfxgc, 'ajjbqk': ajjbqk, 'labels': all_label})
                    else:
                        data.append({'crime': crime, 'query': q, 'ajName': ajName, 'cpfxgc': cpfxgc, 'ajjbqk': ajjbqk, 'labels': all_label})
                else:
                    all_label = [qidx, didx]
                    test_data.append({'crime': crime, 'query': q, 'ajName': ajName, 'cpfxgc': cpfxgc, 'ajjbqk': ajjbqk, 'labels': all_label})
        if data_type == 'train':
            print(len(data))
    ...
    def __getitem__(self, index):
        ...
        q_crime_tokens, d_crime_tokens = self.tokenizer.tokenize(data['crime']), self.tokenizer.tokenize(data['cpfxgc'])
        ...
        # if total_length > 1533:
        #     for _ in range(total_length - 1533):
