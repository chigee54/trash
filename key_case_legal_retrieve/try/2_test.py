...
case_data_path = os.path.join(os.path.dirname(__file__), 'data/case2_test.json')
...
def predict():
    ...
    case_model.load_state_dict(torch.load('result/case/seed2-bsz24-lr1e-05/bert.pt'))  # loss_0.007_dev_0.9697 epoch_7
    case_dataloader = DataLoader(CreateDataset(case_data_path, 'case'), batch_size=128, shuffle=False)
    case_model.eval()
    with torch.no_grad():
        all_preds, info = {}, {}
        for case_data in tqdm(case_dataloader):
            case_data = _move_to_device(case_data)
            score, docids = case_model(**case_data)
