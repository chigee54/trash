...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attn_probs = torch.masked_fill(attn_probs, is_index_masked[:, :attn_probs.shape[1], None, None], 0.0)
...
class LongformerEncoder(nn.Module):
    ...
    def _page_rank_google(self, M, T=20, beta=0.85):
        R = (torch.ones(M.shape[0], M.shape[2], 1) / M.shape[2]).to(M.device)
        S = (torch.ones(M.shape[0], M.shape[2], 1) / M.shape[2]).to(M.device)
        trunc_dim = M.shape[2]
        for time in range(T):
            R = beta * M.transpose(1, 2)[..., :trunc_dim] @ R + (1 - beta) * S
        return R
    ...
    def _pad_to_window_size(
        ...
        if padding_len > 0:
            ...
            if gate_mask is not None:
                gate_mask = F.pad(gate_mask, (0, padding_len), value=pad_token_id)
        ...
        return padding_len, input_ids, attention_mask, token_type_ids, position_ids, inputs_embeds, gate_mask
    ...
    def forward(
        ...
        for idx, layer_module in enumerate(self.layer):
            ...
            if use_reduce and len_reduce_list[idx] < hidden_states.shape[1]:
                # batch x head x seq1 x seq2
                attention_raw = layer_outputs[1]
                attn_shape = attention_raw.shape
                # remove pad nodes
                attention_mask_bool_t = (attention_mask[:, None, None, :] / 10000.0 + 1.0).transpose(1, 3)
                attention = attention_raw * attention_mask_bool_t
                if gate_type == 'pagerank':
                    # batch x seq1 x seq2
                    dist = attention.sum(dim=2) / attention.shape[2]
                ...
                gate += gate_mask.unsqueeze(2) * 1000
                gate += attention_mask.unsqueeze(2)

                hidden_states, _, select_idx = self._top_k(gate, len_reduce_list[idx], hidden_states)
...
class LongformerModel(LongformerPreTrainedModel):
    ...
    def forward(
        ...
        padding_len, input_ids, attention_mask, token_type_ids, position_ids, inputs_embeds, gate_mask = self._pad_to_window_size(
            input_ids=input_ids,
            attention_mask=attention_mask,
