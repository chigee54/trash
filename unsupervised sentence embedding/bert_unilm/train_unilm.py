...
from modeling_bert import BertForMaskedLM, UniLM, BertModel
...
def main(seed):
    ...
    parser.add_argument('-num_epochs', default=1000000, type=int)
    parser.add_argument('-learning_rate', default=2e-5, type=float)
    parser.add_argument('-max_grad_norm', default=1.0, type=float)
    parser.add_argument('-warm_up_proportion', default=0, type=float)
    parser.add_argument('-gradient_accumulation_step', default=1, type=int)
    parser.add_argument('-report_step', default=100000, type=int)
    ...
    parser.add_argument('-output_path', default='output/try1/{}'.format(seed), type=str)
    ...
    model = UniLM.from_pretrained(args.bert_path)
    bert = BertModel.from_pretrained(args.bert_path)
    ...
    if args.do_train:
        train(model_n, tokenizer, writer, args)
    if args.do_test:
...
class PrepareDataset(Dataset):
    def __init__(self, args, tokenizer, eval_mode=None):
        ...
        if eval_mode is not None:
            ...
            if args.train_mode == 'unsupervised':
                ...
                self.data = self.load_sts_all_data()
            else:
    ...
    def load_sts_all_data(self):
        logger.info('loading unsupervised sts all data')
        output_path = os.path.dirname('output/')
        train_file_cache = join(output_path, 'all_sts_features.pkl')
        if os.path.exists(train_file_cache) and not self.overwrite_cache:
            with open(train_file_cache, 'rb') as f:
                ...
                return feature_list
        feature_list = []
        with open('output/train_texts.txt', 'r', encoding='utf8') as f:
            lines = f.readlines()
            # lines = lines[:100]
            logger.info("len of train data:{}".format(len(lines)))
            for line in tqdm(lines):
                line = line.strip()
                feature = self.tokenizer(line, max_length=self.max_seq_length,
    ...
    def load_wiki_unsupervised(self):
        ...
        if self.args.multi_dropout is True:
            ...
            train_file_cache = join(output_path, 'train-unsupervise.pkl') if self.args.demo_train is False else join(output_path, 'demo2_train_5w:10.pkl')
        if os.path.exists(train_file_cache) and not self.overwrite_cache:
            with open(train_file_cache, 'rb') as f:
                feature_list = pickle.load(f)
                logger.info("len of train data:{}".format(len(feature_list)))
...
def compute_seq_loss(outputs_seq, pt_batch, tokenizer):
    ...
    correct_seq = torch.sum(torch.eq(a, y_true))
    filter_matrix = (a == y_true)
    # if correct_seq > 5:
    #     ff = open('correct_num_1.txt', 'a', encoding='utf8')
    #     out_token = tokenizer.convert_ids_to_tokens(a[filter_matrix])
    #     out_ori_token = tokenizer.convert_ids_to_tokens(y_true[filter_matrix])
    #     out_seq = tokenizer.convert_ids_to_tokens(a)
    #     out_ori_seq = tokenizer.convert_ids_to_tokens(y_true)
    #     ff.write('{}\n{}\n{}\n{}\n{}\n'.format(correct_seq, out_token, out_ori_token, out_seq, out_ori_seq))
    return loss


def train(model_n, tokenizer, writer, args):
    model = model_n[0]
    ...
    # optimizer_b, scheduler_b = scheduler_with_optimizer(model[1], train_loader, args)
    # adv = FGM(model)
    best = 0
    best_loss = 1
    for epoch in range(int(args.num_epochs)):
        ...
        for batch_idx, data in enumerate(tqdm(train_loader)):
            ...
            out = model(input_ids, token_type_ids, bert_cls[1])
            loss_mlm = compute_seq_loss(out[1], out[2], tokenizer)
            loss = loss_mlm
            optimizer.zero_grad()
            loss.backward()
            step += 1
            ...
            # scheduler.step()
            model.zero_grad()
            logger.info('Epoch[{}/{}], loss:{}'.format(epoch + 1, args.num_epochs, loss.item()))
            if loss.item() <= best_loss:
                torch.save(model.state_dict(), join(args.output_path, f'unilm_loss_{best_loss}.pt'))
                logger.info('save loss {} model, step {}, epoch {}, save model\n'.format(best_loss, step, epoch + 1))
                best_loss -= 0.1
