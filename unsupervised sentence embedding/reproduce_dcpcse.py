...
# +-------+-------+-------+-------+-------+--------------+-----------------+-------+
# | 72.36 | 85.29 | 76.47 | 83.76 | 80.19 |    81.05     |      70.09      | 78.46 |
# +-------+-------+-------+-------+-------+--------------+-----------------+-------+
...
def main():
    ...
    parser.add_argument('-cls_with_mlp', default=False, type=bool)
    ...
    parser.add_argument('-loss', default='CL', choices=['CL', 'CS', 'SCD'], type=str)
    parser.add_argument('-train_mode', default='unsupervised', choices=['supervised', 'unsupervised'], type=str)
...
def da_function(y_pred, device, permute_num, noise_num):
...
def evaluate(model, dataloader, args):
    ...
    with torch.no_grad():
        for source, target, label in dataloader:
            ...
            source_pred = model(source_input_ids, source_attention_mask, source_token_type_ids, args.cls_with_mlp)
            ...
            target_pred = model(target_input_ids, target_attention_mask, target_token_type_ids, args.cls_with_mlp)
