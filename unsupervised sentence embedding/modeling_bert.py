    ...
    def forward(
        ...
        if self.training:
            ...
            if not self.config.multi_dropout:
                ...
                embeddings_clone = einops.rearrange(embeddings, '(h i) j t -> h i j t', h=len(self.dropout)+1).clone()
    ...
    def forward(
        ...
        if self.training:
            if not self.config.multi_dropout:
                ...
                attention_probs_clone = einops.rearrange(attention_probs, '(h i) j t k -> h i j t k', h=len(self.dropout)+1).clone()
                for h in range(len(self.dropout)+1):
